...
FILE *getAsphaltImage(FILE *fp ,int id){
  char asphalt[25];
  if(id < 10)
    sprintf(asphalt, "../data/DataSet/asphalt/asphalt_0%d.txt",id);
  else
    sprintf(asphalt, "../data/DataSet/asphalt/asphalt_%d.txt",id);

  printf("Arquivo: %s\n", asphalt);
  fp = fopen(asphalt ,"r");

  if(fp==NULL){
    printf("Falha leitura arquivo %d .\n", id);
    exit(1);
  }
  return fp;
}
...
FILE *getGrassImage(FILE *fp ,int id){
  char grass[25];
  if(id < 10)
    sprintf(grass, "../data/DataSet/grass/grass_0%d.txt",id);
  else
    sprintf(grass, "../data/DataSet/grass/grass_%d.txt",id);
